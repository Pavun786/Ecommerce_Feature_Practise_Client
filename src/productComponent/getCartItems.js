...
function GetCartItems(){
  ...
  const getItems = async()=>{
      ...
      const value = await fetch(`http://localhost:4600/cart/allCartItems/${userId}`,{
        method : "GET"
      })
  ...
  const removeItem = async(id)=>{
     ...
     const value = await fetch(`http://localhost:4600/cart/${id}`,{
        method : "DELETE"
     }) 
...
const makeOrder = async() =>{
    ...
    const createData = await fetch('http://localhost:4600/order/create',{
        method : "POST",
        headers : {
...
const EmptyTheCart = async()=>{
     console.log("execute")
     const remove = await fetch(`http://localhost:4600/cart/emptyTheCart/${userId}`,{
        method : "DELETE"
     })
